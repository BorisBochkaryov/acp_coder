...
-type 'TriggerData'() :: ['TriggerID'()].
-type 'Extensions'() :: ['ExtensionField'()].
-type 'ExtensionField'() :: list().

-type 'ExportLegsType'() :: [{SourceLeg::'DTLegID'(), NewLeg::'DTLegID'()}].
...
-type 'AccessCode'() :: 'LocationNumber'().
...
-record('Replaces', {call_id :: string(), remote_tag :: string(), local_tag :: string(), args = [] :: [{Key :: atom(), Value :: term()}]}).
-type 'Replaces'() :: #'Replaces'{}.

-record('ReferTo', {refer_number :: 'Digits'(), args = [] :: [{Key :: atom(), Value :: term()}]}).
...
-type 'AdditionalInformation'() :: {forISUP, 'AdditionalISUP'()} |
                                   ...
                                   {msInfo, 'MSInfo'()} |
                                   {dtmf, integer()} | %% refs #11522. Peer to peer DTMF transit.
                                   {forOther, integer()} |
                                   {pickup_number, _} |
                                   ...
                                   {isFocus, boolean()} |
                                   {replaces, 'Replaces'()} |
                                   {replaces_cp, 'ReplacesCP'()} |
                                   {refer_to, 'ReferTo'()}.

-type 'ListOfAdditionalInformation'() :: ['AdditionalInformation'()].
...
-type 'AdditionalIvrInfo'() :: {Key :: binary(), Value :: binary()}. %% Key :: <<"widget_id">>
...
-record('SSNotification',
        {family     :: 'SSFamily'(),
         ...
         args = []   :: [{Key :: atom(), Value :: term()}]
        }).
-type 'SSNotification'() :: #'SSNotification'{}.

-type 'AdditionalCallingInfo'() :: [{Key :: atom(), Value :: term()}].

-type 'MSKey'() :: correlationId | toneId.

-type 'MSInfo'() :: [{Key :: 'MSKey'(), Value :: term()}]. %% Information for MediaService usage
...
-type 'ISUPCallReference'() :: [integer()]. %% list of bytes
...
-type 'ChargeNumber'() :: 'LocationNumber'().
...
-type 'CountersValue'() :: ['CounterAndValue'()]. %SEQUENCE SIZE(0..numOfCounters) OF CounterAndValue
...
-type 'DestinationRoutingAddress'() :: ['CalledPartyNumber'()].
                                                %	SEQUENCE SIZE(1..numOfAddresses) OF CalledPartyNumber
%%% Indicates the list of Called Party Numbers (primary and alternates).

-type 'Digits'() :: [integer()]. %OCTET STRING (SIZE (minDigitsLength..maxDigitsLength))
...
-type 'FeatureCode'() :: 'LocationNumber'().
...
-type 'GenericNumber'() :: list() | 'GenericNumber-ISUP'(). % OCTET STRING (SIZE(
                                                %minGenericNumberLength..
                                                %maxGenericNumberLength))
%%% Refer to Q.763 Generic Number for encoding.

-type 'GenericNumbers'() :: ['GenericNumber'()]. % SET SIZE(1..numOfGenericNumbers) OF GenericNumber
...
-type 'INServiceCompatibilityIndication'() :: ['Entry'()]. %SEQUENCE SIZE (1..numOfInServiceCompatibilityIndLength) OF Entry
...
-type 'LocationNumber'() :: list(). %OCTET STRING (SIZE (
...
-type 'MidCallControlInfo'() :: [#'MidCallControlInfo'{}].
...
-type 'RequestedInformationList'() :: ['RequestedInformation'()]. %SEQUENCE SIZE (1..numOfInfoItems) OF RequestedInformation

-type 'RequestedInformationTypeList'() :: ['RequestedInformationType'()]. %SEQUENCE SIZE (1..numOfInfoItems) OF RequestedInformationType
...
-type 'RequestedUTSIList'() :: ['RequestedUTSI'()].% SEQUENCE SIZE
                                                %(minRequestedUTSINum..
...
-type 'ServingAreaID'() :: 'LocationNumber'().
...
-type 'TraceInformation'() :: ['TraceItem'()]. %SEQUENCE OF TraceItem

-type 'TraceItem'() :: {scf, 'ScfID'()}.

-type 'TravellingClassMark'() :: 'LocationNumber'().
...
-type 'TriggerResults'() :: ['TriggerResult'()]. %SEQUENCE SIZE (1..numOfTriggers) OF TriggerResult
...
-type 'Triggers'() :: ['Trigger'()]. % SEQUENCE SIZE(1..numOfTriggers) OF Trigger
...
-type 'USIInformation'() :: list(). % OCTET STRING (SIZE
...
-type 'GenericNotificationIndicatorList'() :: ['GenericNotificationIndicator'()].
...
-record('MLPPPrecedence',
        {
          lfb = <<2>> :: binary(), %% <<2>> - look ahead for busy (LFB) not allowed
          precedenceLevel :: undefined | 'PrecedenceLevel'(),
          network_identity :: undefined | binary(),
          mlpp_service_domain :: undefined | binary()
        }).

-type 'MLPPPrecedence'() :: #'MLPPPrecedence'{}.

-type 'PrecedenceLevel'() :: '1_flash_override' | '2_flash' | '3_immediate' | '4_priority' | '5_routine'.
