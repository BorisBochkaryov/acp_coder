...
encode_record(#'CallProgressType'{} = Term) ->
  ...
  EventTime          = ?ENCODE_UNKNOWN(Term#'CallProgressType'.eventTime),
  <<?R_CallProgressType,
    Cause/binary,
...
encode_record(#'ReleaseType'{} = Term) ->
  ...
  EventTime        = ?ENCODE_UNKNOWN(Term#'ReleaseType'.eventTime),
  <<?R_ReleaseType,
    Cause/binary,
    CauseInitiator/binary,
    CauseDescription/binary,
...
encode_record(#'SetupIRType'{} = Term) ->
  ...
  EventTime                  = ?ENCODE_UNKNOWN(Term#'SetupIRType'.eventTime),
  <<?R_SetupIRType,
    Domain/binary,
...
encode_record(#'SetupCRType'{} = Term) ->
  ...
  EventTime          = ?ENCODE_UNKNOWN(Term#'SetupCRType'.eventTime),
  <<?R_SetupCRType,
...
decode_record(<<?R_CallProgressType, Bin/binary>>) ->
  ...
  {EventTime, Bin18}          = decode_unknown(Bin17),
  {#'CallProgressType'{
    cause              = Cause,
...
decode_record(<<?R_ReleaseType, Bin/binary>>) ->
  ...
  {EventTime, Bin11}       = decode_unknown(Bin10),
  {#'ReleaseType'{
    cause            = Cause,
    causeInitiator   = CauseInitiator,
    causeDescription = CauseDescription,
...
decode_record(<<?R_SetupCRType, Bin/binary>>) ->
  ...
  {EventTime, Bin8}          = decode_unknown(Bin7),
  {#'SetupCRType'{
...
decode_record(<<?R_SetupIRType, Bin/binary>>) ->
  ...
  {EventTime, Bin27}                 = decode_unknown(Bin26),
  {#'SetupIRType'{
    domain                     = Domain,
...
writeToLogs(TimeStart, Packet, Result) ->
  ...
  log:info([acpCoder],"~nTime: | Size before,after: | Compress:~n~p~n",[Calc(TimeStart,Packet,Result)]).
